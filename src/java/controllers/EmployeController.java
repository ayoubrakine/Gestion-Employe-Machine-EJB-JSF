...
public class EmployeController implements Serializable {
    ...
    private void loadTree() {
        for (Employe employe : getItems()) {
            TreeNode employeNode = new DefaultTreeNode("Employé : "+employe.getNom() + " " + employe.getPrenom(), root);

            for (Machine machine : employe.getMachineList()) {
                new DefaultTreeNode("Marque :   "+machine.getMarque() + "Référence :   " + machine.getReference(), employeNode);
            }
        }
    }
